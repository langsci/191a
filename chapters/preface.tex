...
\rowcolor{yellow!20!white}
Southern Bantoid: & \textbf{\color{red}Bantu} & Beboid & Yemne-Kimbi & Ekoid\\
\rowcolor{yellow!20!white}
...
\lsptoprule
\cellcolor{green}*Western BC &\cellcolor{yellow} *Eastern BC\\
\midrule 
\cellcolor{green}Nupoid &\cellcolor{yellow} Kainji\\
\cellcolor{green}Defoid &\cellcolor{yellow} Platoid\\
\cellcolor{green}Edoid &\cellcolor{yellow} Cross\\
\cellcolor{green}Igboid &\cellcolor{yellow} Jukunoid\\
\cellcolor{green}Idomoid &\cellcolor{yellow} \textbf{\color{red}Bantoid}\\
\multicolumn{2}{c}{Isolated BC: Oko,\index{Oko!} Akpes,\index{Akpes!} Ikaan,\index{Ikaan!} Lufu\index{Lufu!}}\\
\lspbottomrule
\end{tabularx}
\end{table}
...
The following case may serve as an illustration to this statement. Suppose we need to assess one of Greenberg’s proposals, e.g. a Niger-Congo root meaning ‘hill’. Among the reflexes quoted by Greenberg for this root are: “(2) Busa\index{Busa!} \textit{kpi} ‘mountain’, Kweni  \textit{kpi~}; (4) G{\~{a}}\index{Gã!} \textit{kpɔ~}; Gwa\index{Gwa!} \textit{ogba} ‘mountain’; (5) Nungu\index{Nungu!} \textit{agbɔ}, Ninzam (Ninzo\index{Ninzo!}) \textit{igbu}. Kordofanian: (2) Tagoi\index{Tagoi!} \textit{(c}\textit{)ibe}.” \citep[155]{Greenberg1966}. The phonetic correspondences underlying the comparison of these forms will not be discussed here (we will just assume that they are valid), for the main problem is elsewhere. A reader with no access to a representative lexical database on the NC languages is always uncertain about a number of key issues, including:
...
Chapter 4 deals with the reconstruction of the Proto-Niger-Congo\index{Proto-Niger-Congo!} numeral system on the basis of the step-by-step-reconstructions offered in Chapter 3 for each of the twelve major families and a handful of isolates.  

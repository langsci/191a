\chapter{Reconstruction of Numerals in Niger-Congo}
 
\section{‘One’} 
The five stems present in Table 4.1 are the most likely candidates for the reconstruction of ‘one’ in NC (\tabref{tab:4:1}):
...
The reconstruction of *\textit{ti} ‘1’ is the least certain among the roots discussed above. The form \textit{ha-nthe} ‘1’ attested in the Limba\il{Limba} language isolate is noteworthy.

The last root is a tentative representation of the forms with the initial labio-velar (or labial in the case of Western NC) that are not necessarily etymologically related. The root \textit{ɡu{\`{u}}ŋ} ‘1’ attested in the Gola\il{Gola} isolate may belong here as well. 

In addition to the five roots treated above, apparent innovations may be attested in particular families (or even in groups within them). Among these are Kordofanian \textit{ʈɔn} (cf. Sua\il{Sua} \textit{sɔn}), Gur \textit{t{\'{u}}r{\'{u}}/} \textit{tumɔ}, Mande West \textit{kelen}, and Atlantic Bak -\textit{anor,} \textit{əkon}.
...
\numcolfive{kok/kek/cik, \mbox{(can/ɽan, rak, rɔm)}}{ra(k)/ ra(p),  gba/ gwa}{pa ? ba(i) ?}
}{isolated roots}
...
\numcoltwo{sakpa/sagba/sawa, ʔààkɔ̃/yààká?}{~}
}{%
\numcolthree{~}{~}{~}
}{%
\numcolfour{~}{~}
...
The table below (\tabref{tab:4:6}) provides an overview of the pertinent Bantu reflexes of \textit{*ta\'{}t{\`{ʊ}}} (ABEFGHJKLMNPRS) / \textit{*ca\'{}t{\`{ʊ}}} / \textit{*ca\'{}c{\`{ʊ}}}   (CD) ‘three’ (these reconstructions follow BLR3):
...
\begin{figure}
\begin{tikzpicture}
\node[minimum width=2.5cm, minimum height=.7cm, inner sep=3pt, red] (tat)   {tat};
\node[minimum width=2.5cm, minimum height=.7cm, inner sep=3pt] (at) [right=of tat]   {tat};
\node[minimum width=2.5cm, minimum height=.7cm, inner sep=3pt] (ta) [left=of tat]   {tat};

\node[minimum width=2.5cm, minimum height=.7cm, inner sep=3pt, blue] (CC) [above=of tat,yshift=-13mm]   {C- = -C};
\node[minimum width=2.5cm, minimum height=.7cm, inner sep=3pt, blue] () [right=of CC]   {-C > Ø};
\node[minimum width=2.5cm, minimum height=.7cm, inner sep=3pt, blue] () [left=of CC]   {C- > Ø};
...
Each of them can be transformed, for example, with a spirantisation \textbf{*t} > \textbf{s,} \textbf{or} \textbf{*t} > \textbf{r,} \textbf{*t} > \textbf{l}, can become voiced \textbf{*t} > \textbf{d} and only after that can the second consonant be dropped. (Schemes 4.2-4.3).

\begin{figure}
\begin{tikzpicture}
\node[minimum width=1.5cm, minimum height=.7cm, inner sep=3pt, red] (tat)   {tat};
...
\node[minimum width=1.5cm, minimum height=.7cm, inner sep=3pt, blue] (CC) [above=of tat,yshift=-13mm]   {C- = -C};
\node[minimum width=1.5cm, minimum height=.7cm, inner sep=3pt, blue] (r1) [right=of CC]   {-C > -C’};
\node[minimum width=1.5cm, minimum height=.7cm, inner sep=3pt, blue] (r2) [right=of r1]   {C- > Ø};
\node[minimum width=1.5cm, minimum height=.7cm, inner sep=3pt, blue] (l1) [left=of CC]   {C- > C-’};
\node[minimum width=1.5cm, minimum height=.7cm, inner sep=3pt, blue] (l2) [left=of l1]   {-C > Ø};
...
\end{itemize}
The following table (\tabref{tab:4:7}) provides a structured overview of the derived Bantu forms (with no arrows):
...
\begin{figure}
\begin{tikzpicture}
\node[minimum width=1.5cm, minimum height=.7cm, inner sep=3pt, red] (sat)   {sat};
\node[minimum width=1.5cm, minimum height=.7cm, inner sep=3pt, red] (tat) [above=of sat,yshift=-5mm]  {tat};
...
\caption{\textit{Scheme} \textit{4.5}}
\end{figure}


Examples from seven branches of NC are given below and divided into two structurally identical tables (\tabref{tab:4:9}-4.10):
...
\numcolfive{-ɽɔŋ /-ɽandɔ/ -rʊm?,(-ɡʌ́lʌ̀m)}{~}{~}
}{}
...
\kppyramid
\numcolcomplete{%
\numcolone{tɔk, tən?}{kə-ʈamaʈ (<*kə-ʈa ‘hand’?), tan?}
}{%
\numcoltwo{**tan? (> ‘10’?)}{~}
}{%
\numcolthree{~}{tɔ}{ton}
...
\midrule
Bariba\il{Bariba} & \citealt{Koelle1963} & Baatonum\il{Baatonum} & nóma & n{\'{\={ọ}}}wu\\
Bwamu\il{Bwamu} & \citealt{BloemartsdeRasilly2012} & Bwamu\il{Bwamu} & n{\'{u}}umánnu & ~\\
Grusi & \citealt{Koelle1963} & Tem\il{Tem} & ~ & n{\'{\=o}}n{\={u}}a\\
Grusi & \citealt{CLNK1999} & Kabiye\il{Kabiye} & ~ & naanʋwa\\
Grusi & \citealt{Koelle1963} & Kiamba\il{Kiamba} & noon / noozi & noonuua\\
Grusi & \citealt{Koelle1963} & Sisaala\il{Sisaala} Tumulung & ~ & {\'{n}}n{\textsubbar{\={o}}}m\\
Oti-Volta & \citealt{Koelle1963} & Mosi\il{Mosi} & nur{\textsubbar{o}} & nu\\
Oti-Volta & \citealt{Koelle1963} & Gurma\il{Gurma} & unu / inui & mu {\textasciitilde} mmu\\
\lspbottomrule
\end{tabularx}
\end{table}

This raises the question, are we dealing with direct Dogon-Gur contact or with the reflexes of an additional NC root for ‘hand’? The following roots may be considered potential correspondences: Proto-Bantu\il{Proto-Bantu} \textit{*na\`{}ma\`{}} ‘limb: arm; leg; thigh’ (Regions 4: NW SW Ce NE ; Zones 6: ABEHMR) or \textit{*n{\`{ʊ}}e\`{}} ‘finger, toe’ (Regions 5: NW SW Ce NE SE\il{SE} ; Zones 9: ADJKLMPRS), (\textcyrillic{с}f. Bantu, zones MN – Nyiha-Malila\il{Malila}-Lambya (\citealt{NursePhilippson1975}) \textit{i-nyove}, cf. \citep{Koelle1963} Aku\il{Aku} (Defoid) \textit{ɲɔwɔ} ‘hand’. The Bak (Atlantic) root \textit{ñen} ‘hand’, ‘five’ discussed above may belong here as well. The Gola\il{Gola} root \textit{n{\`{ɔ}}{\`{ɔ}}n{\`{ɔ}}ŋ} should also be mentioned here. The meaning ‘hand’ is not attested for this root in Kwa\il{Kwa} and Adamawa.
...
Atlantic-Bak & Bijogo\il{Bijogo} Kamona & kɔ-kɔ / ŋa-kɔ & ŋu-du$\beta ɔ$-kɔ & ŋ{\'{ɔ}}-r{\'{u}}ŋa-kɔ\\
...
Atl.-Centre-Manjak\il{Manjak} & Mankanya\il{Mankanya} & \textbf{{\'{u}}l{\={o}}l} & kány{\textsubbar{\={e}}}n\\
...
The following Kordofanian terms that attest to the development of ‘hand’ > ‘5’ are also noteworthy: Dagik\il{Dagik} (Kordofanian) \textit{si-s-ɜlːʊ} ‘5’ (litː ‘one hand’): “The \textit{si} in 5 comes from the word `hand'. So\il{So} 5 is `one hand'”\footnote{John Vanderelst, \href{https://mpi-lingweb.shh.mpg.de/numeral/Dagik.htm}{https://mpi-lingweb.shh.mpg.de/numeral/Dagik}\il{Dagik}\href{https://mpi-lingweb.shh.mpg.de/numeral/Dagik.htm}{.htm}}, Acheron\il{Acheron} \textit{zəɡuŋ} \textit{zulluk} (lit: `one hand' ): “The number `five' is literally ‘one hand’: \textit{zəguŋ} = ‘hand’, \textit{z-ulluk} = ‘one’”\footnote{Russell Norton, \href{https://mpi-lingweb.shh.mpg.de/numeral/Acheron.htm}{https://mpi-lingweb.shh.mpg.de/numeral/Acheron}\il{Acheron}\href{https://mpi-lingweb.shh.mpg.de/numeral/Acheron.htm}{.htm}}. 
...
\end{tabularx}
\end{table}
In Dii\il{Dii} (Adamawa-Duru\il{Duru}) a step-by-step replacement of classes is used as a derivation mechanism, i.e. ‘2’ > ‘4’ > ‘8’: \textit{i-d{\'{u}}} ‘2’ > \textit{nda-dd{\'{ʉ}}} ‘4’ > \textit{ka-ʔa-nda-dd{\'{ʉ}}}~ ‘8’.
